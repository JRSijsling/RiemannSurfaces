...
{ Printing. }
        vprint RS,1 : "Spanning tree between",STree`Length+1,"points";
        vprint RS,1 : "with edges:",STree`Edges;
        if assigned STree`DE_Params then
                vprint RS,1 : "Integration parameters (DE):",STree`DE_Params;
        end if;
        if assigned STree`GJ_Params then
                vprint RS,1 : "Integration parameters (GJ):",STree`GJ_Params;
        end if;
end intrinsic;
