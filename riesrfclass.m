...
{ Print the Riemann surface X. }
        if X`IsSuperelliptic then
                vprint RS,1 : "Superelliptic Riemann surface of genus",X`Genus ,"defined as degree",X`Degree[1],"covering defined by",X`DefiningPolynomial,"and prescribed precision",X`Prec;
        else
                vprint RS,1 : "Riemann surface of genus", X`Genus ,"defined by: 0 =",X`DefiningPolynomial,"and prescribed precision",X`Prec;
        end if;
        vprint RS,1 : "";
        vprint RS,1 : "Computed data:";
        vprint RS,1 : " Complex fields:",[Precision(X`ComplexFields[k]):k in [1..#X`ComplexFields]];
        vprint RS,1 : " Discriminant points:", assigned X`DiscriminantPoints;
        vprint RS,1 : " BasePoint:", X`BasePoint;
        vprint RS,1 : " Path pieces:", assigned X`PathPieces;
        vprint RS,1 : " Index path lists:", assigned X`IndexPathLists;
        vprint RS,1 : " Closed chains:", assigned X`ClosedChains;
        vprint RS,1 : " Branch points:", assigned X`BranchPoints;
        vprint RS,1 : " Local monodromy:", assigned X`LocalMonodromy;
        vprint RS,1 : " Monodromy group:", assigned X`MonodromyGroup;
        vprint RS,1 : " Homology basis:", assigned X`HomologyBasis;
        vprint RS,1 : " Holomorphic differential:", assigned X`HolomorphicDifferentials;
        vprint RS,1 : " Integration method:", X`IntMethod;
        vprint RS,1 : " BigPeriod matrix:",assigned X`BigPeriodMatrix;
        vprint RS,1 : " SmallPeriod matrix:",assigned X`SmallPeriodMatrix;
        vprint RS,1 : " Reduction matrix (real)",assigned X`ReductionMatrixReal;
        vprint RS,1 : " Reduction matrices (complex)",assigned X`ReductionMatricesComplex;
        vprint RS,1 : " SheetToSheetIntegrals:", assigned X`SheetToSheetIntegrals;
end intrinsic;
...
{ Create a Riemann surface object defined by f(x,y) = 0 over a number field using the embedding sigma. }
        ...
        if X`Genus eq 3 and Degree(f) eq 4 then
            ...
            fx := Derivative(f, 1);
            fy := Derivative(f, 2);
            fxev := Evaluate(fx, [ KU.1, KU.2 ]);
            fyev := Evaluate(fy, [ KU.1, KU.2 ]);

            vprint RS,1 : "";
            vprint RS,1 : fx;
            vprint RS,1 : (1/3)*fxev;
            vprint RS,1 : Differential(KU.2);
            vprint RS,1 : ((1/3)*fxev)^(-1)*Differential(KU.2) eq X`DFF[3];
            vprint RS,1 : Parent(((1/3)*fxev)^(-1)*Differential(KU.2)) eq Parent(X`DFF[3]);
            vprint RS,1 : "";

            b3 := (1/fyev)*Differential(KU.1);
            b3 := (1/fxev)*Differential(KU.2);
            b1 := (KU.1)*b3;
            b2 := (KU.2)*b3;
            X`DFF := [ b1, b2, b3 ];
            */
        ...
        if X`Genus ne #InnerFaces then
                ...
                vprint RS,1 : "DFF_Factors:",DFF_Factors;
...
{ Creates an superelliptic Riemann surface object with defining equation y^m = p(x). }
        ...
        if not IsExact(K) then
                ...
                if Prec lt 30 then
                        Prec := 30;
                        vprint RS,1 : "Precision has been increased to 30 decimal digits.";
                end if;
        end if;
        X`Prec := Max(Prec,30);
        X`ComplexFields := [ ComplexField(X`Prec) ];
